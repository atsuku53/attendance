<div class="index__container">
    <div class="index-title__block">
        <h1 class="index-title__content">{{ $staff->name }}さんの勤怠</h1>
    </div>
                ...
                @foreach (($monthlyRows ?? []) as $row)
                    ...
                    <tr>
                        ...
                        <td class="attendance-detail__block">
                            <a href="/admin/attendance/detail/{{ $attendance ? $attendance->id : 'add/' . $staff->id . '/' . $row['date']->locale('ja')->format('Y-m-d') }}" class="attendance-detail__link">詳細</a>
                        </td>
                    </tr>
                @endforeach
            </table>
    ...
    <form class="csv-button__block" action="/admin/attendance/staff/{{ $staff->id }}/{{ \Carbon\Carbon::parse($displayMonth . '/1')->format('Y-m') }}/export" method="POST">
        @csrf
        <button type="submit" class="csv-button__content">CSV出力</button>
    </form>
</div>
